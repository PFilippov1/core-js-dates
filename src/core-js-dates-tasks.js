...
function getTime(date) {
  return new Date(date).toTimeString().split(' ')[0].toString();
}
...
function getNextFriday(date) {
  ...
  newDate.setUTCDate(newDate.getUTCDate() + (curDay >= 5 ? 7 : 0) + 5 - curDay);
  return newDate;
}
/**
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const shiftTime = Math.abs(end - start);
  const shiftDays = Math.ceil(shiftTime / (1000 * 60 * 60 * 24)) + 1;
  return shiftDays;
}
